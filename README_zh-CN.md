...
- Windows（x64）
- 一台 MIDI 设备
- [Node.js](https://nodejs.org/) 22+ —— 打包后的程序需要通过系统的 `node` 来启动后端扩展，且随包分发的原生模块（`node-midi`、`koffi`）是按 Node.js 22 的 ABI 编译的。请使用**与发布版本相同的 Node 主版本（22）**；若未安装 Node 或主版本不符，GUI 将无法开始监听（后端日志中会给出明确错误提示）。

> 说明：若未来发布版本自带 Node 运行时，此项要求将不再需要——在此之前，尽管便携版 `.exe` 很便捷，但应用**并非完全自包含**。
...
- **新增映射**：
  - **MIDI 音符**：点击输入框后弹奏 MIDI 琴键自动填入音符编号（需提前选择好 MIDI 设备）
  - **输出键位**：通过顶部切换捕捉模式
    - **单键**：点击输入框后按下单个按键（a / enter / f1 等）自动填入
    - **组合键**：点击输入框后依次按下各个按键（如 ctrl、shift、escape），自动拼合为组合键
  - 填写完毕后点击 **添加** 完成添加
